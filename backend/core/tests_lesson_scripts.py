...
class LoginStillWorksEveryWay(TestCase):
    """🔑 লগইনের প্রতিটি পথ — আইডি, ইমেইল, মোবাইল।

    ⚠️ লগইনের কোডে হাত দেওয়া সবচেয়ে ঝুঁকির। এখানে ক্রম বদলানো হয়েছে
    (আগে সোজা পথে চেষ্টা, না হলে ইমেইল/মোবাইল খোঁজা) — একটি ডাটাবেজ
    প্রশ্ন বাঁচাতে। যাচাইয়ের নিয়ম বদলায়নি, কিন্তু তা প্রমাণ করা চাই।
    """
    ...
    def setUp(self):
        ...
        self.User = User
        self.cl = APIClient()
        self.u = User.objects.create(username="ustadh", role="teacher",
                                     email="ustadh@tqa.test", phone="01711000000")
        self.u.set_password(self.PW)
        self.u.save()

    _KEEP = object()          # "পাসওয়ার্ড দেওয়া হয়নি" বোঝাতে

    def login(self, ident, pw=_KEEP):
        """⚠️ `pw or self.PW` লেখা যাবে না — তাতে খালি পাসওয়ার্ড দিলেও
        আসলটা বসে যায়, আর "খালি পাসওয়ার্ডে ঢোকা যায় না" পরীক্ষাটি
        মিথ্যা পাস করে। একবার ঠিক এই ভুলেই বিভ্রান্ত হয়েছিলাম।"""
        if pw is self._KEEP:
            pw = self.PW
        return self.cl.post("/api/auth/login",
    ...
    def test_email_works(self):
        r = self.login("ustadh@tqa.test")
        self.assertEqual(r.status_code, 200, "ইমেইল দিয়ে লগইন ভেঙেছে")

    def test_phone_works(self):
        r = self.login("01711000000")
        self.assertEqual(r.status_code, 200, "মোবাইল দিয়ে লগইন ভেঙেছে")

    def test_username_is_case_insensitive(self):
        r = self.login("USTADH")
        self.assertEqual(r.status_code, 200, "বড় হাতের আইডিতে ভেঙেছে")

    def test_email_is_case_insensitive(self):
        r = self.login("Ustadh@TQA.test")
        self.assertEqual(r.status_code, 200, "বড় হাতের ইমেইলে ভেঙেছে")

    def test_spaces_around_the_id_are_ignored(self):
        r = self.login("  ustadh  ")
        self.assertEqual(r.status_code, 200, "ফাঁকসহ আইডিতে ভেঙেছে")

    # ───── যেসব পথে ঢোকা যাবে না ─────
    def test_wrong_password_is_refused(self):
        self.assertEqual(self.login("ustadh", "ভুল").status_code, 401)

    def test_wrong_password_with_email_is_refused(self):
        self.assertEqual(self.login("ustadh@tqa.test", "ভুল").status_code, 401)

    def test_unknown_user_is_refused(self):
        self.assertEqual(self.login("keu-na").status_code, 401)

    def test_empty_password_is_refused(self):
        self.assertEqual(self.login("ustadh", "").status_code, 400)

    def test_an_inactive_user_cannot_enter(self):
        self.u.is_active = False
        self.u.save()
        self.assertEqual(self.login("ustadh").status_code, 401,
                         "নিষ্ক্রিয় ব্যবহারকারী ঢুকে পড়ছেন")

    # ───── একাধিক অ্যাকাউন্টে একই নম্বর ─────
    def test_a_shared_phone_picks_the_right_account(self):
        """⚠️ অভিভাবকের একই নম্বর দুই সন্তানের অ্যাকাউন্টে থাকতে পারে।"""
        other = self.User.objects.create(username="kid", role="student",
                                         phone="01711000000")
        other.set_password("OtherPass!99")
        other.save()
        r = self.login("01711000000", "OtherPass!99")
        self.assertEqual(r.status_code, 200, "দ্বিতীয় অ্যাকাউন্টে ঢোকা যায় না")
        r2 = self.login("01711000000", self.PW)
        self.assertEqual(r2.status_code, 200, "প্রথম অ্যাকাউন্টে ঢোকা যায় না")

    # ───── খরচ ─────
    def test_the_common_path_costs_one_query(self):
        """⚠️ আসল লাভ — আইডি দিয়ে লগইনে একটিমাত্র প্রশ্ন।"""
        from django.test.utils import CaptureQueriesContext
        from django.db import connection
        with CaptureQueriesContext(connection) as q:
            self.login("ustadh")
        self.assertLessEqual(len(q), 1,
                             "আইডি দিয়ে লগইনে %d প্রশ্ন — একটিই হওয়ার কথা"
                             % len(q))

    def test_the_email_path_stays_cheap_too(self):
        from django.test.utils import CaptureQueriesContext
        from django.db import connection
        with CaptureQueriesContext(connection) as q:
            self.login("ustadh@tqa.test")
        self.assertLessEqual(len(q), 3, "ইমেইল পথে %d প্রশ্ন" % len(q))


class NobodyEntersAnotherAccount(TestCase):
    """🔒 সবচেয়ে জরুরি পাহারা — কেউ যেন অন্যের আইডিতে বা পাসওয়ার্ড ছাড়া
    ঢুকতে না পারে।

    ⚠️ লগইনের ক্রম বদলানো হয়েছে গতি বাড়াতে। যাচাইয়ের নিয়ম বদলায়নি,
    কিন্তু এখানে সেটাই হাতে-কলমে প্রমাণ করা হয় — প্রতিটি ফাঁক ধরে ধরে।
    """
    ...
    def setUp(self):
        ...
        from rest_framework.test import APIClient
        self.cl = APIClient()
        self.a = User.objects.create(username="rahim", role="student",
                                     email="rahim@tqa.test", phone="01711111111")
        self.a.set_password("RahimPass!11")
        self.a.save()
        self.b = User.objects.create(username="karim", role="teacher",
                                     email="karim@tqa.test", phone="01722222222")
        self.b.set_password("KarimPass!22")
        self.b.save()

    def try_login(self, ident, pw):
        return self.cl.post("/api/auth/login",
                            {"username": ident, "password": pw},
                            format="json")

    def whoami(self, r):
        """টোকেনটা আসলে কার — সেটাই আসল প্রশ্ন।"""
        from rest_framework.test import APIClient
        c = APIClient()
        c.credentials(HTTP_AUTHORIZATION="Bearer " + r.data["access"])
        return c.get("/api/users/me/").data.get("username")

    def test_the_token_belongs_to_the_right_person(self):
        for ident in ("rahim", "rahim@tqa.test", "01711111111"):
            r = self.try_login(ident, "RahimPass!11")
            self.assertEqual(r.status_code, 200, ident)
            self.assertEqual(self.whoami(r), "rahim",
                             "“%s” দিয়ে ঢুকে অন্যের অ্যাকাউন্ট পেয়েছে!" % ident)

    def test_one_persons_password_does_not_open_another(self):
        """⚠️ রহিমের পাসওয়ার্ড দিয়ে করিমের আইডিতে ঢোকা যাবে না।"""
        for ident in ("karim", "karim@tqa.test", "01722222222"):
            r = self.try_login(ident, "RahimPass!11")
            self.assertEqual(r.status_code, 401,
                             "“%s”-এ অন্যের পাসওয়ার্ডে ঢোকা গেছে!" % ident)

    def test_no_password_never_works(self):
        """⚠️ পাসওয়ার্ড ছাড়া কোনোভাবেই নয়।"""
        for pw in ("", " ", None):
            for ident in ("rahim", "rahim@tqa.test", "01711111111"):
                r = self.try_login(ident, pw)
                self.assertNotEqual(
                    r.status_code, 200,
                    "“%s” + পাসওয়ার্ড %r দিয়ে ঢোকা গেছে!" % (ident, pw))

    def test_no_username_never_works(self):
        for ident in ("", " ", None):
            r = self.try_login(ident, "RahimPass!11")
            self.assertNotEqual(r.status_code, 200,
                                "আইডি %r ছাড়াই ঢোকা গেছে!" % ident)

    def test_an_unknown_person_cannot_enter(self):
        for ident in ("keu-na", "keu@na.test", "01799999999"):
            self.assertEqual(self.try_login(ident, "RahimPass!11").status_code,
                             401, "নেই এমন কেউ ঢুকে পড়ছে: " + ident)

    def test_a_shared_phone_gives_each_their_own_account(self):
        """⚠️ অভিভাবকের একই নম্বর দুই সন্তানের অ্যাকাউন্টে — প্রত্যেকে যেন
        নিজেরটাই পান, অন্যেরটা নয়।"""
        self.b.phone = "01711111111"          # দুজনের একই নম্বর
        self.b.save()
        r1 = self.try_login("01711111111", "RahimPass!11")
        self.assertEqual(r1.status_code, 200)
        self.assertEqual(self.whoami(r1), "rahim", "ভুল অ্যাকাউন্ট খুলেছে")
        r2 = self.try_login("01711111111", "KarimPass!22")
        self.assertEqual(r2.status_code, 200)
        self.assertEqual(self.whoami(r2), "karim", "ভুল অ্যাকাউন্ট খুলেছে")

    def test_a_shared_phone_still_refuses_a_wrong_password(self):
        self.b.phone = "01711111111"
        self.b.save()
        self.assertEqual(
            self.try_login("01711111111", "একেবারেই-ভুল").status_code, 401,
            "শেয়ার করা নম্বরে ভুল পাসওয়ার্ডেই ঢোকা গেছে!")

    def test_an_inactive_account_stays_shut(self):
        self.a.is_active = False
        self.a.save()
        for ident in ("rahim", "rahim@tqa.test", "01711111111"):
            self.assertEqual(self.try_login(ident, "RahimPass!11").status_code,
                             401, "নিষ্ক্রিয় অ্যাকাউন্টে ঢোকা গেছে: " + ident)
