...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_due_months(self, obj):
        """যে মাসগুলোর টাকা সত্যিই বাকি।

        💰 ⚠️ মওকুফ করা মাস এখানে আসে না। মওকুফ করলে DueMonth সারিটি
        মুছে ফেলা হয় না, কেবল waived=True বসে — ইচ্ছাকৃত, যাতে "মওকুফ"
        আর "পরিশোধিত" আলাদা করে দেখানো যায় আর মাসিক কাজটি ওই মাসকে
        নতুন করে বকেয়া না বানায়।

        আগে এখানে সব সারিই ফেরত যেত, মওকুফসহ। ফলে পরিচালকের "যাদের ফি
        বাকি" পর্দায় মওকুফ করা মাসও বকেয়া দেখাত, আর টাকার অঙ্কও
        (মাস × ফি) বেড়ে যেত — সেই ভুল অঙ্ক WhatsApp রিমাইন্ডারে
        অভিভাবকের কাছেও চলে যেত।

        ⚠️ .filter(waived=False) লেখা যাবে না — সেটি
        prefetch_related("due_months")-এর ক্যাশ এড়িয়ে প্রতি ব্যবহারকারীতে
        নতুন কোয়েরি করত (N+1)। তাই .all() ইটারেট করে পাইথনেই ছেঁকে
        নেওয়া হয়; সারিগুলো তো ক্যাশে আছেই, বাড়তি খরচ নেই।
        """
        return [d.month_label for d in obj.due_months.all() if not d.waived]
