...
class ClassSessionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if u.role == "teacher":
            ...
            return qs.filter(
                # 🔒 ⚠️ Q(course__teacher=u) ইচ্ছা করেই সরানো হয়েছে।
                # কোর্সের নির্ধারিত উস্তাদ হলেই ওই কোর্সের *সব* ক্লাস দেখা
                # যেত — এক কোর্সে দুজন উস্তাদ পড়ালে একজন আরেকজনের ক্লাস ও
                # শিক্ষার্থীর নাম দেখে ফেলতেন। পরিচালকের নিয়ম: হিসাব কেবল
                # কোন শিক্ষার্থী কোন উস্তাদের কাছে পড়ে, কোর্স ধরে নয়।
                # রুটিনের ছাঁকনিতে এই শর্তটি কখনোই ছিল না, তাই দুটোর আচরণও
                # আলাদা ছিল — এখন দুটোই এক নিয়মে চলে।
                Q(teacher=u) | Q(students__teacher=u)
            ).distinct()
        if u.role == "student":
            return qs.filter(students=u)
        # ⚠️ ট্রায়াল অতিথিকে আলাদা করে ধরতেই হবে। নিচের শেষ "return qs"
    ...
    def finish(self, request, pk=None):
        ...
        ১ম পর্ব শেষ (রিজয়েন এখনো খোলেনি) —
          ...
          • শিক্ষার্থীদের কাছে রিজয়েন লিংক খুলে যায় (join_mode_override),
            ফলে তাঁদের পোর্টালে রিজয়েন বাটন চলে আসে — সাথে সাথে নয়,
            কয়েক সেকেন্ডের মধ্যে (প্যানেলে থাকলে ৫ সেকেন্ডের পোলে,
            তালিকায় থাকলে ২০ সেকেন্ডের পোলে; জুম থেকে ট্যাবে ফিরলে
            তখনই)
...
class LessonViewSet(viewsets.ModelViewSet):
    ...
    def seed_sample(self, request):
        """লেখা দারসটি কোনো কোর্সে বসিয়ে দেওয়া।

        ⚠️ সামনের পর্দায় এর কোনো বোতাম আর নেই — "নমুনা দারস" ব্যবস্থাটি
        তুলে দেওয়া হয়েছে, দারসগুলো এখন সরাসরি আসল দারস হিসেবেই থাকে
        (মাইগ্রেশন ০০৩৬)। পথটি রাখা হয়েছে দুটি কারণে: মাইগ্রেশন একই
        কোড ব্যবহার করে, আর নতুন কোর্সে দারসটি বসানোর দরকার হলে যেন
        উপায় থাকে।
