...
/* ☀️ ডাটাবেজ জাগানো — অ্যাপ খোলার সাথে সাথেই, একবার।

   ⚠️ কেন দরকার: Neon নিষ্ক্রিয় থাকলে কম্পিউট বন্ধ করে দেয়। পরের প্রথম
   প্রশ্নটি ডাটাবেজ জাগা পর্যন্ত অপেক্ষা করে — ১০-২০ সেকেন্ডও লাগতে
   পারে। ঠিক তখনই পরিচালক "দারস আনা যায়নি" দেখতেন, অথচ কোডে কোনো
   সমস্যা ছিল না। অ্যাপ খুললেই আগেভাগে জাগিয়ে রাখলে সেই দেরিটা
   ব্যবহারকারীর চোখেই পড়ে না।

   ⚠️ ব্যর্থ হলেও কিছু যায় আসে না — চুপচাপ ছেড়ে দিই। এটি কেবল আগেভাগে
   জাগানোর চেষ্টা, কোনো তথ্য এখান থেকে আসে না।

   ⚠️ ক্রন নয় — কেউ অ্যাপ খুললে তবেই একবার চলে। আগে প্রতি মিনিটের একটি
   ক্রন Neon-এর কম্পিউট-কোটা পুড়িয়ে ফেলেছিল; এখানে সে ঝুঁকি নেই। */
let warmed = false;
export function warmUp() {
  if (warmed) return;
  warmed = true;
  try {
    fetchWithTimeout(`${BASE}/warmup/`, { method: "GET" }, 25000).catch(
      () => {},
    );
  } catch {
    /* উপেক্ষা — জাগানোর চেষ্টা ব্যর্থ হলেও অ্যাপ আগের মতোই চলবে */
  }
}
