...
const syllabusHTML = (courseName, booksLine, itemsByCat) => {
  ...
  const col = (cat) => {
    ...
    const lis = items.length
      ? items.map((it) => {
          const bk = it.book && it.book !== "অন্যান্য" ? `<span class="bk">${esc(it.book)} — </span>` : "";
          const pl = (it.pages ? ` · পৃষ্ঠা: ${esc(it.pages)}` : "") + (it.lines ? ` · লাইন: ${esc(it.lines)}` : "");
          const note = it.note ? `<div class="nt">💬 ${esc(it.note)}</div>` : "";
...
const openPrintDoc = (html, filename) => {
  ...
  } catch (e) { notice("প্রিন্ট খুলতে সমস্যা হয়েছে।"); }
};
...
const SYL_CATEGORIES = [
  { key: "memorized_surah",  label: "মুখস্থ সূরা",                 icon: "📖", book: true,  placeholder: "যেমন: সূরা ইখলাস" },
  { key: "memorized_hadith", label: "মুখস্থ হাদিস",                icon: "📜", book: true,  placeholder: "যেমন: ১ম হাদিস — নিয়ত" },
  { key: "qirat",            label: "কিরাত",                       icon: "🕋", book: true,  placeholder: "যেমন: কায়দা — লেসন ৪" },
  { key: "dua_masala",       label: "দুআ/মাসআলা",                  icon: "🤲", book: true,  placeholder: "যেমন: খাবারের দুআ" },
  { key: "moral_story",      label: "নৈতিক শিক্ষা/হাদিসের গল্প",   icon: "🌟", book: true,  placeholder: "যেমন: সততার গল্প" },
];
...
function LecturePlan({ db, courses, user, refresh }) {
  ...
  const openNew = () => setForm({ mode: "new", no: String(lectures.length + 1), title: "", selIds: [] });
  const openEdit = (lec) => setForm({ mode: "edit", lecId: lec.id, no: String(lec.no), title: lec.title, selIds: lec.topics.map((t) => t.syllabusId).filter(Boolean) });
  ...
  const saveForm = async () => {
    ...
    if (!form.selIds.length) return notice("সিলেবাস থেকে অন্তত একটি বিষয় বাছাই করুন।");
    const noVal = parseInt(form.no, 10);
    try {
      if (form.mode === "new") {
        await api.createLecture(sel, form.title.trim(), form.selIds, noVal ? { no: noVal } : {});
      } else {
        await api.editLecture(form.lecId, { title: form.title.trim(), syllabus_item_ids: form.selIds, ...(noVal ? { no: noVal } : {}) });
      }
      await loadData();
    } catch {
      ...
      if (form.mode === "new") {
        ...
        setLectures((prev) => [...prev, { id: uid(), no: noVal || prev.length + 1, title: form.title.trim(), topics, date: null }]);
      } else {
        setLectures((prev) => prev.map((l) => l.id === form.lecId ? { ...l, no: noVal || l.no, title: form.title.trim(), topics: picked.map((s) => { const old = l.topics.find((t) => t.syllabusId === s.id); return old ? { ...old, text: sylLabel(s) } : { id: uid(), syllabusId: s.id, text: sylLabel(s), covered: null }; }) } : l));
      }
    }
    setForm(null);
  };
  ...
  return (
    <Section title="দৈনিক পাঠ পরিকল্পনা ও টপিক কভারেজ" sub="পরিচালক দারস তৈরি করবেন · কভার করা বিষয় ✔ সবুজ · বাদ পড়া ✘ লাল — লাল ক্রস কেবল এডমিন/পরিচালক ঠিক করবেন"
      ...
      <div style={{ ...S.card, marginBottom: 12, display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 180 }}>
          ...
          <div style={S.sub}>উস্তাদ: {userById(course.teacherId || course.teacher)?.name || "—"} · মোট দারস: {bn(lectures.length)}</div>
        </div>
      ...
      {lectures.length === 0 && (
        <div style={{ ...S.card, textAlign: "center", color: C.muted, padding: 28 }}>
          📋 এই কোর্সের দৈনিক পাঠ পরিকল্পনা এখনো তৈরি হয়নি।{isDir(user) ? " ওপরের \"+ দারস যোগ করুন\" বাটন দিয়ে শুরু করুন।" : " পরিচালক তৈরি করলে এখানে দেখা যাবে ইনশাআল্লাহ।"}
        </div>
      )}
      <div style={{ display: "grid", gap: 10 }}>
        {lectures.map((lec) => {
      ...
      {form && (
        <Modal title={form.mode === "new" ? `+ নতুন দারস — ${course.name}` : `✏️ দারস এডিট (টিক/ক্রস অক্ষত থাকবে)`} onClose={() => setForm(null)} wide>
          <div style={{ display: "grid", gridTemplateColumns: "120px 1fr", gap: 10 }}>
            <div><label style={S.label}>দারস-নং</label>
              <input type="number" min="1" style={S.input} value={form.no} onChange={(e) => setForm({ ...form, no: e.target.value })} placeholder="যেমন: ৫" /></div>
            <div><label style={S.label}>দারসের শিরোনাম</label>
              <input style={S.input} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="যেমন: হরকত — যবর, যের, পেশ" /></div>
          </div>
          <div style={{ marginTop: 12 }}>
            <label style={S.label}>সিলেবাস থেকে বিষয় বাছাই করুন — {bn(form.selIds.length)}টি নির্বাচিত (বিভাগ অনুযায়ী সাজানো)</label>
            {sylList.length === 0 ? (
              <div style={{ padding: "14px 12px", borderRadius: 10, background: C.amberBg, fontSize: 12.5, color: "#a16207" }}>
                ⚠️ "{course.name}" কোর্সের সিলেবাস এখনো তৈরি হয়নি — আগে 📜 সিলেবাস মেনুতে গিয়ে প্রতিটি বিভাগে বিষয় যোগ করুন, তারপর এখানে বাছাই করতে পারবেন।
              </div>
            ) : (
              <div style={{ display: "grid", gap: 10 }}>
                {SYL_CATEGORIES.map((cat) => {
                  const items = sylList.filter((s) => (s.category || "qirat") === cat.key);
                  if (items.length === 0) return null;
                  return (
                    <div key={cat.key} style={{ border: `1.5px solid ${C.line}`, borderRadius: 12, overflow: "hidden" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: C.cream, borderBottom: `1px solid ${C.line}` }}>
                        <span style={{ fontSize: 15 }}>{cat.icon}</span>
                        <b style={{ flex: 1, fontSize: 13, color: C.emerald }}>{cat.label}</b>
                        <span style={{ fontSize: 11.5, color: C.muted }}>{bn(items.filter((s) => form.selIds.includes(s.id)).length)}/{bn(items.length)}</span>
                      </div>
                      <div style={{ padding: 6 }}>
                        {items.map((s) => (
          ...
          <Btn style={{ marginTop: 16, width: "100%", justifyContent: "center" }} onClick={saveForm}>{form.mode === "new" ? "+ দারস যোগ করুন" : "✏️ সংরক্ষণ করুন"}</Btn>
        </Modal>
      )}
    </Section>
  );
...
function AttendanceView({ db, courses, user }) {
  ...
  useEffect(() => {
    api.classes().then(setSessions).catch(() => setSessions(null));
  }, []);

  let rows;
  if (sessions) {
    rows = sessions.flatMap((k) =>
      (k.attendance || []).map((a) => ({
        name: a.user_name || "—",
        courseName: k.course_name || courseById(courses, k.course || k.courseId).name,
        date: k.date, minutes: a.minutes || 0, userId: a.user,
      }))
    )
    .filter((r) => user.role === "student" ? String(r.userId) === String(user.id) : true)
    .map((r) => {
      const ok = r.minutes >= 40;
      return [r.name, r.courseName, fmtDate(r.date), `${bn(r.minutes)} মিনিট`, ok ? <Tag key="t">উপস্থিত ✔</Tag> : <Tag key="t" color={C.red} bg={C.redBg}>অনুপস্থিত (৪০ মিনিটের কম)</Tag>];
    });
  } else {
    rows = db.attendance
      .map((a) => ({ a, k: db.classes.find((k) => k.id === a.classId) }))
      .filter(({ k }) => k && courseById(courses, k.courseId).id)
      .filter(({ a }) => user.role === "student" ? a.userId === user.id : true)
      .map(({ a, k }) => {
        const c = courseById(courses, k.courseId);
  ...
  return (
    <Section title="হাজিরা রিপোর্ট" sub="ন্যূনতম ৪০ মিনিট ক্লাসে থাকলে তবেই হাজিরা গণ্য হয়">
      <Table head={["নাম", "কোর্স", "তারিখ", "উপস্থিতি", "অবস্থা"]} rows={rows} empty="কোনো হাজিরা রেকর্ড নেই" />
    </Section>
  );
}
...
function ProgressView({ db, setDb, courses, user }) {
  const [allStudents, setAllStudents] = useState(user.role === "student" ? [user] : USERS.filter((u) => u.role === "student"));
  ...
  const loadData = async () => {
    try {
      ...
      const [feesData, duesData, sessData, examsData, studsData] = await Promise.all(tasks);
      setFees(feesData.map((p) => ({ id: p.id, studentId: p.student || p.studentId, amount: +p.amount, month: p.month_label || p.month, date: p.paid_at || p.date, method: p.method + (p.trx_id ? ` (Trx: ${p.trx_id})` : ""), status: p.status })));
      const dm = {};
      duesData.forEach((d) => { const uid2 = String(d.user || d.userId); if (!dm[uid2]) dm[uid2] = []; dm[uid2].push(d.month_label || d.month); });
      setDuesMap(dm);
      setSessions(sessData);
      setExamList(examsData.map((e) => ({ ...e, courseId: e.course || e.courseId, total: e.total_marks || e.total, marks: e.results || e.marks || {} })));
      if (studsData) setAllStudents(studsData.map((s) => ({ id: s.id, name: s.name || s.name_bn, sub: s.sub || s.sub_title, fee: s.monthly_fee || s.fee, role: "student" })));
    } catch { /* keep mock */ }
  };
  useEffect(() => { loadData(); }, [user.id]);

  const st = allStudents.find((s) => String(s.id) === String(sel)) || userById(sel);
  const selId = String(sel);
  const stCourses = courses.filter((c) => (c.studentIds || c.students || []).some((x) => String(x) === selId));
  const att = sessions
    ? sessions.flatMap((k) => (k.attendance || []).filter((a) => String(a.user) === selId).map((a) => ({ minutes: a.minutes || 0 })))
  ...
  const makeups = (db.makeups || []).filter((m) => String(m.studentId) === selId);
  const exams = examList.filter((e) => e.marks && e.marks[sel] != null);
  const recordPay = async () => {
    const month = dueMonths[0]; if (!month) return;
    try {
      await api.payFee({ amount: st.fee, month_label: month, method: "নগদ গ্রহণ (অফিস)" });
      await loadData();
    } catch {
      setFees((prev) => [...prev, { id: uid(), studentId: sel, amount: st.fee, month, date: todayISO(), method: "নগদ গ্রহণ (অফিস)", status: "verified" }]);
      setDuesMap((prev) => ({ ...prev, [selId]: (prev[selId] || []).slice(1) }));
    }
  };
  const studentPay = async () => {
    const month = dueMonths[0]; if (!month) return;
    try {
      await api.payFee({ amount: st.fee, month_label: month, method: pf.method, trx_id: pf.trx });
      await loadData();
    } catch {
      setFees((prev) => [...prev, { id: uid(), studentId: sel, amount: st.fee, month, date: todayISO(), method: pf.method + (pf.trx ? ` (Trx: ${pf.trx})` : ""), status: "pending" }]);
      setDuesMap((prev) => ({ ...prev, [selId]: (prev[selId] || []).slice(1) }));
    }
    setPay(false); setPf({ method: "বিকাশ", trx: "" });
  };
  const verifyPay = async (pid) => {
    try { await api.verifyFee(pid); await loadData(); }
    catch { setFees((prev) => prev.map((p) => p.id === pid ? { ...p, status: "verified" } : p)); }
  };
  return (
    <Section title="শিক্ষার্থীর অগ্রগতি ও রিপোর্ট" sub="অগ্রগতি · ফি · মিসিং ক্লাস · মেকআপ ক্লাস — সব এক জায়গায়" action={isAdm(user) && <Btn kind="gold" sm onClick={() => setMaker(true)}>🧾 রিসিট বানান</Btn>}>
      ...
      {user.role !== "student" && (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
          {allStudents.map((s) => <Btn key={s.id} sm kind={String(sel) === String(s.id) ? "primary" : "soft"} onClick={() => setSel(s.id)}>{s.name}</Btn>)}
        </div>
      )}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 10, marginBottom: 14 }}>
        ...
        <Stat icon="⏳" label="বকেয়া" value={`৳${bn(due.toLocaleString("en"))}`} accent={C.red} note={dueMonths.join(", ") || "নেই"} />
      </div>
      <div style={{ display: "grid", gap: 14 }}>
        ...
        <div style={{ ...S.card }}>
          ...
          <Table head={["মাস", "পরিমাণ", "তারিখ", "মাধ্যম", "অবস্থা", "রিসিট"]} rows={myFees.map((p) => [p.month, `৳${bn((+p.amount).toLocaleString("en"))}`, fmtDate(p.date), p.method,
            p.status === "pending"
            ...
            <Btn key="r" sm kind="soft" onClick={() => printReceipt({ ...p, date: fmtDate(p.date) }, st, "ফি পরিশোধ রিসিট")}>🧾 PDF</Btn>])} empty="কোনো পেমেন্ট নেই" />
          {pay && (
            <Modal title={`ফি পরিশোধ — ${dueMonths[0] || ""}`} onClose={() => setPay(false)}>
...
function AccountsView({ db, setDb, user }) {
  ...
  const loadData = async () => {
    try {
      const [feesData, salData, usersData, duesData] = await Promise.all([api.myFees(), api.salaries(), api.allUsers(), api.myDues()]);
      setFees(feesData.map((p) => ({ ...p, amount: +p.amount })));
      setSalaries(salData.map((p) => ({ ...p, teacherId: p.teacher || p.teacherId, amount: +p.amount, month: p.month_label || p.month, teacher_name: p.teacher_name })));
      setTeachers(usersData.filter((u) => u.role === "teacher").map((u) => ({ id: u.id, name: u.name || u.name_bn, salary: u.monthly_salary || u.salary || 0, role: "teacher" })));
      const dm = {};
      duesData.forEach((d) => { const k = String(d.user || d.userId); if (!dm[k]) dm[k] = []; dm[k].push(d.month_label || d.month); });
      setDuesMap(dm);
    } catch { /* keep mock */ }
  };
  useEffect(() => { loadData(); }, []);
  ...
  const payTeacher = async (t) => {
    ...
    const month = dues[0]; if (!month) return;
    try {
      await api.sendReceipt({ to_user: t.id, kind: "বেতন পরিশোধ ভাউচার", month_label: month, amount: t.salary || t.monthly_salary, method: "ব্যাংক" });
      await loadData();
    } catch {
      setSalaries((prev) => [...prev, { id: uid(), teacherId: t.id, teacher_name: t.name, amount: t.salary, month, date: todayISO(), method: "ব্যাংক" }]);
      setDuesMap((prev) => ({ ...prev, [String(t.id)]: (prev[String(t.id)] || []).slice(1) }));
    }
  };
  return (
    <Section title="হিসাব-নিকাশ" sub="আয় (স্টুডেন্ট ফি) · ব্যয় (উস্তাদদের বেতন) · বকেয়া" action={<Btn kind="gold" onClick={() => setMaker(true)}>🧾 রিসিট বানান</Btn>}>
      ...
      <div style={{ ...S.card, marginBottom: 14 }}>
        ...
        <Table head={["নাম", "মাসিক বেতন", "বকেয়া মাস", "অ্যাকশন"]}
          rows={teachers.map((t) => {
            const dues = duesMap[String(t.id)] || db.dueMonths?.[t.id] || [];
            return [t.name, `৳${bn((t.salary || t.monthly_salary || 0).toLocaleString("en"))}`, dues.length ? <Tag key="d" color={C.red} bg={C.redBg}>{dues.join(", ")}</Tag> : <Tag key="d">পরিশোধিত ✔</Tag>, dues.length ? <Btn key="b" sm kind="gold" onClick={() => payTeacher(t)}>পেমেন্ট দিন</Btn> : "—"];
          })} />
      </div>
      <div style={{ ...S.card, marginBottom: 14 }}>
        <div style={{ fontWeight: 800, marginBottom: 10 }}>📤 বেতন পরিশোধের ইতিহাস</div>
        <Table head={["উস্তাদ", "মাস", "পরিমাণ", "তারিখ", "মাধ্যম", "ভাউচার"]} rows={salaries.map((p) => {
          const tName = p.teacher_name || userById(p.teacherId || p.teacher)?.name || "—";
          return [tName, p.month || p.month_label, `৳${bn((+p.amount || 0).toLocaleString("en"))}`, fmtDate(p.paid_at || p.date), p.method, <Btn key="r" sm kind="soft" onClick={() => printReceipt({ ...p, month: p.month || p.month_label, date: fmtDate(p.paid_at || p.date) }, { name: tName }, "বেতন পরিশোধ ভাউচার")}>🧾 PDF</Btn>];
      ...
      <div style={{ ...S.card }}>
        ...
        <Table head={["স্টুডেন্ট", "মাসিক ফি", "বকেয়া মাস", "বকেয়া টাকা"]}
          rows={Object.entries(duesMap).filter(([, months]) => months.length > 0).map(([sid, months]) => {
            const feeData = fees.find((p) => String(p.student || p.studentId) === sid);
            const monthlyFee = feeData?.amount || 0;
            const name = USERS.find((u) => String(u.id) === sid)?.name || "স্টুডেন্ট " + sid;
            return [name, `৳${bn(monthlyFee.toLocaleString("en"))}`, months.join(", "), <Tag key="t" color={C.red} bg={C.redBg}>৳{bn((months.length * monthlyFee).toLocaleString("en"))}</Tag>];
...
function FormsView({ db, setDb }) {
  ...
  useEffect(() => {
    api.admissions().then((data) => {
      const contactForms = data.filter((a) => a.kind === "trial" || a.kind === "contact");
      if (contactForms.length > 0) setForms(contactForms.map((a) => ({
        id: a.id, type: a.kind === "trial" ? "ফ্রি ট্রায়াল" : "যোগাযোগ",
        name: a.name, contact: a.contact || a.email, msg: a.message || a.msg,
        date: a.applied_at || a.date, status: a.replied ? "replied" : "new",
      })));
    }).catch(() => { /* keep mock */ });
  }, []);
  ...
  const toggle = async (id) => {
    ...
    setForms((prev) => prev.map((x) => x.id === id ? { ...x, status: newStatus } : x));
  };
  return (
...
function MyReceiptsView({ db, user }) {
  const [receipts, setReceipts] = useState((db.sentReceipts || []).filter((x) => String(x.toUserId) === String(user.id)));

  useEffect(() => {
    api.myReceipts().then((data) => {
      setReceipts(data.map((r) => ({
        id: r.id, toUserId: r.to_user || r.toUserId,
        kind: r.kind, month: r.month_label || r.month,
  ...
  return (
    <Section title="ভাউচার / রিসিট" sub="একাডেমি থেকে আপনার জন্য পাঠানো রিসিট ও ভাউচার — দেখুন, প্রিন্ট বা PDF সেভ করুন">
      <Table head={["ধরন", "মাস/বিবরণ", "পরিমাণ", "তারিখ", "পাঠিয়েছেন", "দেখুন"]}
        rows={receipts.map((x) => [x.kind, x.month, `৳${bn(Number(x.amount).toLocaleString("en"))}`, x.date, x.sentBy || "—",
...
function TeacherWiseBoard({ db, setDb, user }) {
  ...
  useEffect(() => {
    Promise.all([api.allUsers(), api.routines(), api.classes()]).then(([usersData, routData, classData]) => {
      ...
      setUpcomingAll(db.classes.filter((k) => k.date >= todayISO() && k.status === "upcoming"));
    });
  }, []);

  const tid = user.role === "teacher" ? user.id : sel;
  const t = allTeachers.find((x) => String(x.id) === String(tid)) || userById(tid);
  const myRoutines = routines.filter((r) => String(r.teacherId || courseById(COURSES, r.courseId)?.teacherId) === String(tid));
  const upcoming = upcomingAll.filter((k) => String(k.teacher || k.teacherId || courseById(COURSES, k.course || k.courseId)?.teacherId) === String(tid)).sort((a, b) => (a.date + a.time).localeCompare(b.date + b.time)).slice(0, 6);

  const postpone = (k) => askConfirm("ক্লাসটি স্থগিত করবেন? উস্তাদ, স্টুডেন্ট সবার পোর্টালে সাথে সাথে আপডেট হবে এবং অভিভাবকের WhatsApp মেসেজ তৈরি হবে।", async () => {
    try {
      ...
      setDb((d) => ({ ...d,
        ...
        waOutbox: [...waGuardianMsgs(k, c, "postpone"), ...(d.waOutbox || [])],
        notifications: [{ id: uid(), for: [tid, ...studs, "admin1", "dir1"], text: `⛔ ${c.name} ক্লাসটি (${fmtDate(k.date)}, ${k.time}) স্থগিত করা হয়েছে।`, date: todayISO(), read: false }, ...d.notifications] }));
      setUpcomingAll((prev) => prev.filter((x) => x.id !== k.id));
    }
  });
    ...
    <div style={{ ...S.card, marginBottom: 16 }}>
      ...
      {user.role !== "teacher" && (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
          {allTeachers.map((x) => <Btn key={x.id} sm kind={String(tid) === String(x.id) ? "primary" : "soft"} onClick={() => setSel(x.id)}>{x.name}</Btn>)}
        </div>
      )}
      <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6, color: C.emerald }}>📚 {t.name}-এর কাছে যারা পড়ে:</div>
      {myRoutines.length === 0 && <div style={{ fontSize: 12.5, color: C.muted, marginBottom: 8 }}>এখনো রুটিন নেই</div>}
      {myRoutines.map((r) => {
...
/* ═══════════════ কোর্স সিলেবাস — রোল-ভিত্তিক স্ট্যাকড ভিউ · পরিচালক এডিট/যোগ · সবাই দেখে + প্রিন্ট ═══════════════ */
function SyllabusView({ db, user }) {
  // নিজস্ব API থেকে কোর্স ও সিলেবাস লোড — global COURSES খালি থাকলেও কাজ করে
  const [allCourses, setAllCourses] = useState([]);
  const [syllabus, setSyllabus] = useState({});   // { courseId: [items] }
  ...
  const adaptSyl = (s) => ({
    id: s.id, courseId: String(s.course || s.courseId), category: s.category || "qirat",
    book: s.book_name || s.book || "অন্যান্য", lesson: s.lesson || "",
    pages: s.pages || "", lines: s.lines || "", note: s.note || "",
  });

  // কোর্স তালিকা লোড (ব্যাকএন্ড রোল অনুযায়ী ফিল্টার করে)
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        ...
        const mapped = cs.map((c) => ({
          id: String(c.id), name: c.name, teacherId: String(c.teacher || ""),
          color: c.color, books: c.books || [], studentIds: (c.students || []).map(String),
        }));
  ...
  // প্রতিটি কোর্সের সিলেবাস লোড (lazy — কোর্স লিস্ট পাওয়ার পর)
  useEffect(() => {
    if (!allCourses.length) return;
    allCourses.forEach((c) => {
      if (loadedCids.has(c.id)) return;
      setLoadedCids((p) => new Set([...p, c.id]));
      api.syllabus(c.id).then((data) => {
        setSyllabus((prev) => ({ ...prev, [c.id]: data.map(adaptSyl) }));
      }).catch(() => {
        const fallback = (db.syllabus || []).filter((s) => String(s.courseId) === String(c.id)).map(adaptSyl);
        setSyllabus((prev) => ({ ...prev, [c.id]: fallback }));
      });
    });
  }, [allCourses]); // eslint-disable-line

  const reloadCourse = async (cid) => {
    try {
      const data = await api.syllabus(cid);
      setSyllabus((prev) => ({ ...prev, [cid]: data.map(adaptSyl) }));
    } catch { /* keep existing */ }
  };

  // সব একাডেমিক বইয়ের নাম (id→name)
  const bookNameOf = (id) => (allBooks.find((b) => String(b.id) === String(id)) || {}).name;
  // কোর্সের নিজস্ব বইগুলো; খালি হলে সব একাডেমিক বই ব্যবহার
  const booksForCourse = (course) => {
    const cb = (course.books || []).map(bookNameOf).filter(Boolean);
    if (cb.length) return cb;
    return allBooks.map((b) => b.name).filter(Boolean); // ফলব্যাক: সব একাডেমিক বই
  };

  // ── draft helpers ──
  const getDraft = (cid, catKey) => ((draft[cid] || {})[catKey] || {});
  const setD = (cid, catKey, patch) =>
    setDraft((p) => ({ ...p, [cid]: { ...(p[cid] || {}), [catKey]: { ...((p[cid] || {})[catKey] || {}), ...patch } } }));

  // draft init যখন কোর্স ও বই লোড হয়
  useEffect(() => {
    if (!allCourses.length) return;
    setDraft((prev) => {
      const next = { ...prev };
      allCourses.forEach((c) => {
        if (next[c.id]) return; // ইতিমধ্যে আছে — রিসেট নয়
        const books = booksForCourse(c);
        const def = books[0] || "অন্যান্য";
        const d = {};
        SYL_CATEGORIES.forEach((cat) => { d[cat.key] = { book: def, lesson: "", pages: "", lines: "" }; });
        next[c.id] = d;
      });
      return next;
    });
  }, [allCourses, allBooks]); // eslint-disable-line

  // ── দ্রুত যোগ ──
  const addQuick = async (cid, cat) => {
    const dr = getDraft(cid, cat.key);
    if (!(dr.lesson || "").trim()) return;
    const books = booksForCourse(allCourses.find((c) => c.id === cid) || {});
    const bookVal = dr.book || books[0] || "অন্যান্য";
    const isQ = cat.key === "qirat";
    const payload = {
      course: cid, category: cat.key,
      book_name: bookVal,
      lesson: dr.lesson.trim(),
      pages: isQ ? (dr.pages || "").trim() : "",
      lines: isQ ? (dr.lines || "").trim() : "",
      note: "",
    };
    const optimistic = { id: uid(), courseId: cid, category: cat.key, book: bookVal, lesson: payload.lesson, pages: payload.pages, lines: payload.lines, note: "" };
    try { await api.addSyllabus(payload); await reloadCourse(cid); }
    catch { setSyllabus((prev) => ({ ...prev, [cid]: [...(prev[cid] || []), optimistic] })); }
    setD(cid, cat.key, { lesson: "", pages: "", lines: "" });
  };

  // ── এডিট সেভ ──
  const saveEdit = async () => {
    if (!edit.lesson.trim()) return notice("বিষয়বস্তু লিখে দিন।");
    const isQ = edit.category === "qirat";
    const payload = {
      course: edit.courseId, category: edit.category,
      book_name: edit.book || "অন্যান্য",
      lesson: edit.lesson.trim(),
      pages: isQ ? edit.pages.trim() : "",
      lines: isQ ? edit.lines.trim() : "",
      note: edit.note.trim(),
    };
    const entry = { ...edit, book: payload.book_name, lesson: payload.lesson, pages: payload.pages, lines: payload.lines, note: payload.note };
    try { await api.editSyllabus(edit.id, payload); await reloadCourse(edit.courseId); }
    catch { setSyllabus((prev) => ({ ...prev, [edit.courseId]: (prev[edit.courseId] || []).map((s) => s.id === edit.id ? { ...s, ...entry } : s) })); }
    setEdit(null);
  };

  const del = (s) => askConfirm("সিলেবাসের এই অংশটি মুছে ফেলবেন?", async () => {
    try { await api.deleteSyllabus(s.id); await reloadCourse(s.courseId); }
    catch { setSyllabus((prev) => ({ ...prev, [s.courseId]: (prev[s.courseId] || []).filter((x) => x.id !== s.id) })); }
  });

  // ── প্রিন্ট ──
  const doPrint = (course) => {
    const list = syllabus[course.id] || [];
    const cBooks = booksForCourse(course).join(", ");
    const byCat = {};
    SYL_CATEGORIES.forEach((c) => { byCat[c.key] = list.filter((s) => (s.category || "qirat") === c.key); });
    openPrintDoc(syllabusHTML(course.name, cBooks, byCat), `TQA-syllabus-${course.name || "course"}.html`);
  };

  // ── এক কোর্সের সিলেবাস ব্লক ──
  const CourseSyllabusBlock = ({ course }) => {
    const list = syllabus[course.id] || [];
    const books = booksForCourse(course);
    const canEdit = isDir(user);

    // edit modal এর জন্য বই তালিকা
    const editBooks = edit && edit.courseId === course.id ? booksForCourse(allCourses.find((c) => c.id === edit.courseId) || {}) : books;
    ...
    return (
      <div style={{ borderRadius: 14, overflow: "hidden", border: `2px solid ${C.emerald}`, marginBottom: 28 }}>
        ...
        <div style={{ overflowX: "auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${SYL_CATEGORIES.length}, minmax(190px, 1fr))`, minWidth: 950 }}>
            {SYL_CATEGORIES.map((cat, ci) => {
              const items = list.filter((s) => (s.category || "qirat") === cat.key);
              const dr = getDraft(course.id, cat.key);
              return (
                <div key={cat.key} style={{ borderLeft: ci ? `1px solid ${C.line}` : "none", display: "flex", flexDirection: "column" }}>
                  {/* কলাম হেডার */}
                  <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 10px", background: C.greenBg, borderBottom: `2px solid ${C.green}`, position: "sticky", top: 0 }}>
                    <span style={{ fontSize: 14 }}>{cat.icon}</span>
                    <b style={{ flex: 1, fontSize: 12, color: C.emerald, lineHeight: 1.3 }}>{cat.label}</b>
                    <Tag>{bn(items.length)}</Tag>
                  </div>
                  {/* আইটেম লিস্ট */}
                  <ol style={{ margin: 0, padding: "8px 8px 8px 26px", flex: 1, minHeight: 50, background: "#fff" }}>
                    {items.length === 0 && <div style={{ listStyle: "none", marginLeft: -16, textAlign: "center", color: C.muted, fontSize: 12, padding: "10px 0" }}>—</div>}
                    {items.map((s) => (
                      <li key={s.id} style={{ fontSize: 12, marginBottom: 7, lineHeight: 1.5 }}>
                        <span>
                        ...
                        {canEdit && (
                          <span style={{ display: "inline-flex", gap: 3, marginLeft: 4, verticalAlign: "middle" }}>
                            <button title="এডিট" onClick={() => setEdit({ id: s.id, courseId: s.courseId, category: s.category || "qirat", book: s.book || "অন্যান্য", lesson: s.lesson, pages: s.pages || "", lines: s.lines || "", note: s.note || "" })} style={{ border: "none", background: C.cream, borderRadius: 5, width: 20, height: 20, cursor: "pointer", fontSize: 9 }}>✏️</button>
                            <button title="মুছুন" onClick={() => del(s)} style={{ border: "none", background: C.redBg, color: C.red, borderRadius: 5, width: 20, height: 20, cursor: "pointer", fontSize: 9 }}>🗑</button>
                          </span>
                        )}
                      </li>
                    ))}
                  ...
                  {/* পরিচালকের ইনপুট */}
                  {canEdit && (
                    <div style={{ padding: 7, borderTop: `1px dashed ${C.line}`, background: C.cream, display: "grid", gap: 5 }}>
                      {/* সব ক্যাটাগরিতেই বই ড্রপডাউন — memorized_surah/hadith তেও */}
                      <select value={dr.book || books[0] || "অন্যান্য"} onChange={(e) => setD(course.id, cat.key, { book: e.target.value })} style={{ ...S.input, padding: "5px 7px", fontSize: 11.5 }}>
                        {books.map((b) => <option key={b} value={b}>{b}</option>)}
                        <option value="অন্যান্য">অন্যান্য / বই ছাড়া</option>
                      </select>
                      <input value={dr.lesson || ""} onChange={(e) => setD(course.id, cat.key, { lesson: e.target.value })}
                        onKeyDown={(e) => { if (e.key === "Enter") addQuick(course.id, cat); }}
                        placeholder={cat.placeholder} style={{ ...S.input, padding: "6px 8px", fontSize: 12 }} />
                      {cat.key === "qirat" && (
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
                          <input value={dr.pages || ""} onChange={(e) => setD(course.id, cat.key, { pages: e.target.value })} onKeyDown={(e) => { if (e.key === "Enter") addQuick(course.id, cat); }} placeholder="পৃষ্ঠা" style={{ ...S.input, padding: "5px 7px", fontSize: 11.5 }} />
                          <input value={dr.lines || ""} onChange={(e) => setD(course.id, cat.key, { lines: e.target.value })} onKeyDown={(e) => { if (e.key === "Enter") addQuick(course.id, cat); }} placeholder="লাইন" style={{ ...S.input, padding: "5px 7px", fontSize: 11.5 }} />
                        </div>
                      )}
                      <Btn sm style={{ justifyContent: "center" }} onClick={() => addQuick(course.id, cat)}>+ যোগ</Btn>
                    </div>
                  )}
                </div>
              );
        ...
        {canEdit && (
          <div style={{ fontSize: 11, color: C.muted, padding: "6px 14px 8px", background: C.greenBg }}>
            💡 Enter চাপলেই দ্রুত যোগ · বই একবার বাছলে পরপর লেসন দ্রুত যোগ করা যায়
          </div>
        )}
        {/* এডিট মডাল — এই কোর্সের জন্য */}
        {edit && edit.courseId === course.id && (
          <Modal title={`✏️ ${catInfo(edit.category).label} — এডিট`} onClose={() => setEdit(null)}>
            <label style={S.label}>বিভাগ</label>
            <select style={S.input} value={edit.category} onChange={(e) => {
              const cat = e.target.value;
              setEdit({ ...edit, category: cat, book: edit.book || editBooks[0] || "অন্যান্য", pages: cat === "qirat" ? edit.pages : "", lines: cat === "qirat" ? edit.lines : "" });
            }}>
              {SYL_CATEGORIES.map((c) => <option key={c.key} value={c.key}>{c.label}</option>)}
            </select>
            {/* সব বিভাগেই বই সিলেকশন */}
            <div style={{ marginTop: 10 }}>
              <label style={S.label}>বই</label>
              <select style={S.input} value={edit.book || "অন্যান্য"} onChange={(e) => setEdit({ ...edit, book: e.target.value })}>
                {editBooks.map((b) => <option key={b} value={b}>{b}</option>)}
                <option value="অন্যান্য">অন্যান্য / বই নির্ধারিত নয়</option>
              </select>
            </div>
            <div style={{ marginTop: 10 }}>
              <label style={S.label}>{edit.category === "qirat" ? "লেসন / বিষয়বস্তু" : "বিষয়বস্তু"}</label>
              <input style={S.input} value={edit.lesson} onChange={(e) => setEdit({ ...edit, lesson: e.target.value })} placeholder={catInfo(edit.category).placeholder} />
            </div>
            {edit.category === "qirat" && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 10 }}>
                <div><label style={S.label}>পৃষ্ঠা</label><input style={S.input} value={edit.pages} onChange={(e) => setEdit({ ...edit, pages: e.target.value })} placeholder="যেমন: ১২–১৫" /></div>
                <div><label style={S.label}>লাইন</label><input style={S.input} value={edit.lines} onChange={(e) => setEdit({ ...edit, lines: e.target.value })} placeholder="যেমন: ১–৮" /></div>
              </div>
            )}
            <div style={{ marginTop: 10 }}>
              <label style={S.label}>মন্তব্য (ঐচ্ছিক)</label>
              <textarea rows={2} style={{ ...S.input, resize: "vertical" }} value={edit.note} onChange={(e) => setEdit({ ...edit, note: e.target.value })} />
            </div>
            <Btn style={{ marginTop: 16, width: "100%", justifyContent: "center" }} onClick={saveEdit}>✏️ সংরক্ষণ করুন</Btn>
          </Modal>
        )}
      </div>
    );
